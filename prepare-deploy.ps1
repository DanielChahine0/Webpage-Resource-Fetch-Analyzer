...
if (Test-Path $loggerFile) {
    ...
    if ($content -match 'const isDevelopment = true') {
        ...
        Write-Host "  [OK] Development logging disabled" -ForegroundColor Green
    } else {
        Write-Host "  [INFO] Development logging already disabled" -ForegroundColor Cyan
    }
} else {
    Write-Host "  [ERROR] Logger file not found!" -ForegroundColor Red
}
Write-Host ""
...
foreach ($file in $criticalFiles.Keys) {
    if (Test-Path $file) {
        Write-Host "  [OK] $file - $($criticalFiles[$file])" -ForegroundColor Green
    } else {
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "  [INFO] Not a Git repository or Git not installed" -ForegroundColor Cyan
}
Write-Host ""
